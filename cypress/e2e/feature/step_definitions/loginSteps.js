...
Then('the system should display an error message saying {string}', async function (expectedErrorMessage) {
  ...
  const errorMessage = await $('p[class="text-sm text-red-600 dark:text-red-400"]'); // Selector for the error message element
  const actualErrorMessage = await errorMessage.getText();
  
  // Assert that the error message is as expected
  assert.strictEqual(actualErrorMessage, expectedErrorMessage, `Expected error message to be " " but got "These credentials do not match our records."`);
});
